...
const { data } = useFetch<any>(`/api/seo/${id}`);
...
watchEffect(() => {
  ...
  // Jika fetching berhasil (sesuaikan dengan struktur response API kamu)
  if (fetchedData) {
    // 1. Map data utama ke state form
    state.title = fetchedData.title ?? "";
    state.slug = fetchedData.slug ?? "";
    state.content = fetchedData.content ?? "";

    // 2. Map data nested SEO ke state.seo
    if (fetchedData.seo) {
      state.seo.title = fetchedData.seo.title ?? "";
      state.seo.description = fetchedData.seo.description ?? "";

      // Simpan URL gambar lama ke state dan ref preview
      existingOgImage.value = fetchedData.seo.image ?? "";
    }
  }
});
